...
class ModelDownloaderTest {
    ...
    @Test
    fun `downloadFile cleans up partial file on stream read error`() = runTest {
        mockWebServer.enqueue(MockResponse()
            .setResponseCode(200)
            .setBody("test")
        ...
        val targetFileName = "error_test.bin"
        ...
        try {
            downloader.downloadFile(mockContext, url, targetFileName).toList()
        } catch (e: Exception) {
            exceptionThrown = true
        }

        assertTrue("Exception should have been thrown", exceptionThrown)
        assertFalse("Target file should be deleted on error", targetFile.exists())
    }

    @Test
    fun `downloadAndExtractZip cleans up temp zip file on stream read error`() = runTest {
        mockWebServer.enqueue(MockResponse()
            .setResponseCode(200)
            .setBody("test")
            .setSocketPolicy(okhttp3.mockwebserver.SocketPolicy.DISCONNECT_DURING_RESPONSE_BODY)
        ...
        val targetDirName = "error_test_dir"
        val tempZipFile = File(tempDir, "$targetDirName.zip")

        val url = mockWebServer.url("/model.zip").toString()

        var exceptionThrown = false
        try {
            downloader.downloadAndExtractZip(mockContext, url, targetDirName).toList()
        } catch (e: Exception) {
            exceptionThrown = true
        }

        assertTrue("Exception should have been thrown", exceptionThrown)
        assertFalse("Temp zip file should be deleted on error", tempZipFile.exists())
    }
