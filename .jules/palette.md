...
## 2024-05-24 - Button Loading Indicator Visibility
**Learning:** CircularProgressIndicator inside a styled component (like Button) defaults to the primary theme color. When the Button is disabled, its background changes, and the primary color may have poor contrast or be invisible against the disabled background.
**Action:** Always explicitly set the color parameter of an inline CircularProgressIndicator to LocalContentColor.current so it dynamically inherits the parent's contrasting content color.
